...
public class RestPassword extends ModuleRoot {
    ...
    public Object sendPasswordMail() throws NuxeoException {
        ...
        if (errorMessage != null) {
            ...
            if (ctx.getLocale().equals(Locale.FRENCH)) {
                ...
                subject = ctx.getMessage("label.site.name") + " - Votre nouveau mot de passe";
            } else {
                template = getView("mail/passwordForgotten");
                subject = ctx.getMessage("label.site.name") + " - Your new password";
            }
    ...
    @Produces("text/html")
    public Object submitNewPassword() throws NuxeoException,
            URISyntaxException {
        FormData formData = getContext().getForm();
